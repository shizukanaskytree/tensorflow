...
class MSGPackSerializer(SerializerBase):
    ...
    def ext_serializable(cls, type_code: int):
        ...
        def wrap(wrapped_type: type):
            ...
            if type_code in cls._ext_type_codes:
                logger.warning(
                    f"{cls.__name__}: type {type_code} is already registered, overwriting."
                )
            cls._ext_type_codes[type_code], cls._ext_types[wrapped_type] = (
                wrapped_type,
                type_code,
            )
            return wrapped_type
    ...
    def _decode_ext_types(cls, type_code: int, data: bytes):
        if type_code in cls._ext_type_codes:
            ...
            return tuple(
                msgpack.unpackb(data, ext_hook=cls._decode_ext_types, raw=False)
            )
    ...
    def dumps(cls, obj: object) -> bytes:
        return msgpack.dumps(
            obj, use_bin_type=True, default=cls._encode_ext_types, strict_types=True
        )
