...
def switch_to_uvloop() -> asyncio.AbstractEventLoop:
    ...
    :note: The design is:
    1. create a event loop by asyncio package
    2. set the loop as the event loop.
    """
...
async def asingle(aiter: AsyncIterable[T]) -> T:
    ...
    async for item in aiter:
        ...
        if count == 2:
            raise ValueError(
                "asingle() expected an iterable with exactly one item, but got two or more items"
            )
    if count == 0:
        raise ValueError(
            "asingle() expected an iterable with exactly one item, but got an empty iterable"
        )
    return item
...
async def aiter_with_timeout(
    iterable: AsyncIterable[T], timeout: float
) -> AsyncIterator[T]:
...
async def attach_event_on_finished(
    iterable: AsyncIterable[T], event: asyncio.Event()
) -> AsyncIterator[T]:
