...
from atom.utils.mpfuture import InvalidStateError, MPFuture
...
class TaskPool(TaskPoolBase):
    ...
    def _pool_output_loop(self, pending_batches: Dict[Any, List[Task]]):
        ...
        while True:
            ...
            # get `batch_index` and `batch_outputs` from `def send_outputs_from_runtime`
            # `def send_outputs_from_runtime` 是驱
            # `self.outputs_receiver.recv()` 是动
