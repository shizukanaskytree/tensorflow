...
from atom.utils import get_logger
...
class Runtime(threading.Thread):
    ...
    :param model_backends: a dict [expert uid -> ExpertBackend]
    ...
    def __init__(
        ...
        # 这里是变相地存入了所有 model forward, backward 的函数. 等着要被执行. 
        self.pools = tuple(
            chain(*(
                expert.get_pools() for expert in model_backends.values())
            )
        )
    ...
    def run(self):
        ...
        if self.device is not None:
            for expert_backend in self.model_backends.values():
                # 这个可以 Runtime 时决定让谁上 GPU.
