...
def proto_compile(output_path):
    ...
    cli_args = [
        "grpc_tools.protoc",
        "--proto_path=grpctimedstorage/proto",
        f"--python_out={output_path}",
        f"--grpc_python_out={output_path}",
    ] + glob.glob("grpctimedstorage/proto/*.proto")
...
class Develop(develop):
    def run(self):
        super().run()
        proto_compile(os.path.join("grpctimedstorage", "proto"))

setup(
    name='grpctimedstorage', # grpc timed storage 
    version='0.0.1', 
