...
def PmfToQuantizedCdf(pmf, precision):
  return pmf_to_quantized_cdf(pmf=pmf, precision=precision)
tf_export("raw_ops.PmfToQuantizedCdf")(PmfToQuantizedCdf)
...
def range_decode(encoded, shape, cdf, precision, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "RangeDecode", name, _ctx._post_execution_callbacks, encoded, shape,
        cdf, "precision", precision)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RangeDecode(encoded, shape, cdf, precision):
  return range_decode(encoded=encoded, shape=shape, cdf=cdf, precision=precision)
tf_export("raw_ops.RangeDecode")(RangeDecode)
...
def range_encode(data, cdf, precision, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        _ctx._context_handle, _ctx._thread_local_data.device_name,
        "RangeEncode", name, _ctx._post_execution_callbacks, data, cdf,
        "precision", precision)
      return _result
    except _core._FallbackException:
      try:
...
@_doc_controls.do_not_generate_docs
@_kwarg_only
def RangeEncode(data, cdf, precision):
  return range_encode(data=data, cdf=cdf, precision=precision)
tf_export("raw_ops.RangeEncode")(RangeEncode)
