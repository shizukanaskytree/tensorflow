...
class Server(object):
    ...
    server_or_cluster_def 非常约束的条件呐!!!
    这能在这个 cluster 里面才能互相通信是吗?
    可以事后加入这个 cluster 吗?
    """

    def __init__(
        self,
        server_or_cluster_def,  # 这个来自 ClusterSpec
        job_name=None,
        task_index=None,
        protocol=None,
        config=None,
        ...
        # 2021年11月29日
        # 一旦用了这个 cluster setting 传入后, 情况就不妙了.
        # 所以目前的流程就是 先用 DHT 来扫一下有多少机器还在, 然后用这个 数量和其他信息, 作为图切割的输入
        # 然后, 再构造有的这一切.
        # 当情况生变, 这套流程就得再来一遍. 没办法的.
        # 如果是按照这个套路, 那么我感觉还不如用 pytorch 的那套.
