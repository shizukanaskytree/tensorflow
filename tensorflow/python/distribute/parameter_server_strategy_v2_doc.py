...
load_context = LazyLoader(
    "load_context", globals(), "tensorflow.python.keras.saving.saved_model.load_context"
)
...
class ParameterServerStrategyV2(distribute_lib.Strategy):
    ...
    思路:
    Variable are created on parameter servers.
    Variable are read and updated by workers.
    ...
    functions, and coordinating the training.
    ...
    Upon receiving requests from the coordinator, a
    ...
    "without direct interactions with other workers in the cluster." 这里需要改一下.
    ...
    can be occasionally unavailable (e.g. preemptible or spot instances).
    注释: 这个就很吊了. 也有 fail-safe 的机制.

    The coordinator and parameter servers though, must be available at all times for
    ...
    With a CTL, Custom Training Loop
...
class ParameterServerStrategyV2Extended(
    ...
    def _resource_creator_scope(self):
        ...
        with self._coordinator_creation_lock:
            if (
                not self._container_strategy()._cluster_coordinator
