...
/** \brief GraphMgr keeps track of a set of graphs that are registered with a
...
/** \brief Abstract params into Item instance.
 *
 *  \param [in] session: const string&
 *              This parameter is from request message
 *
 *  \param [in] gdef: const GraphDef&
 *              This parameter is from request message
 *
 *  \param [in] graph_options: const GraphOptions&
 *              This parameter is from request message
 *
 *  \param [in] debug_options: const DebugOptions&
 *              This parameter is from request message
 *
 *  \param [in] collective_graph_key: int64
 *              This parameter is from request message
 *
 *  \param [in] cluster_flr: DistributedFunctionLibraryRuntime*
 *
 *  \param [out] item: Item*
 *
 *  \remark Worker::RegisterGraphAsync pass those params from request message.
 *
 */
...
Status GraphMgr::InitItem(const string& session, const GraphDef& gdef,
  ...
  /// OpRegistry::Global()
  /// gdef.library() nothing inside
  /// p item->lib_def->num_functions()
  /// $7 = 0
  item->lib_def.reset(
  ...
  for (auto& p : partition_graphs) {
    ...
    /// 这个 lambda 是用来给 node 创建 OpKernel 的。
...
/** \brief
 *
 *  \param out: [out] NamedTensors*
 *         typedef std::map<string, Tensor> NamedTensors;
 *
 *  \param step_id: const int64
 *
 */
...
/** \brief Worker runs the graph computation. Master ask worker run the graph.
 *
 ...
 *
 *  \param collector: StepStatsCollector*;
 *
 *  \param response: MutableRunGraphResponseWrapper*;
 *
 *  \param cancellation_manager: CancellationManager*;
 *
 *  \param in: const NamesTensors&;
 *
...
void GraphMgr::ExecuteAsync(const string& handle, const int64 step_id,
  ...
  {
    mutex_lock l(mu_);
    /// To find the graph handler, if cannot find, abort.
    auto iter = table_.find(handle);
    if (iter != table_.end()) {
      item = iter->second;
      item->Ref();
...
/** \brief Worker starts running the graph. Master asks worker run the graph.
 *
 *  \param handle: const string&;
 *         graph handler.
 *
 ...
 *
 *  \param ce_handle: CollectiveExecutor::Handle*;
 *
 *  \param collector: StepStatsCollector*;
 *
 *  \param cost_graph: CostGraphDef*;
 *
 *  \param cancellation_manager: CancellationManager*;
 *
 *  \param done: StatusCallback;
...
void GraphMgr::StartParallelExecutors(const string& handle, int64 step_id,
  ...
  /// \note
  /// The ThreadPool::Schedule is to throw the task fn to a bunch of threads.
  /// Unfriendly function def, friendly function form:
  /// void tensorflow::thread::ThreadPool::Schedule	(std::function< void()> fn)
  /// default_runner(_1) is corresponding to the param `fn`.
  ...
  for (const auto& unit : item->units) {
    ...
    /// \note
    ///  unit: struct GraphMgr::ExecutionUnit
    ///  unit.device: class Device
    ///  tensorflow_device_thread_pool() is from class DeviceBase, derived by
    ///    class Device.
    ...
    /// Execute nodes of a Graph in BFS order.
    /// unit.root: Executor*;
