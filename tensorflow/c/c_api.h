...
// wxf
// TF_SetPriority is to set the priority of the current thread, which constructs
// the TF_Graph, ExecutorImpl.
// Set the client primary thread's priority level.
// 
// The definition of the priority level is:
// 0: ExecutorImpl instances generated by ThreadPool threads.
// 1: Low priority ExecutorImpl instances by low priority client primary thread.
// 2: High priority ExecutorImpl instances by high priority client primary
//      thread.
//
// How to use:
// TF_SetExecutionPriority maps to the python API:
//
// ```
// tf.set_execution_priority(execution_priority)
// ```
//
...
// user
// code
// priority level <---> python thread id <---> DirectSession Priority
//           +           (Bridge)                  +
//           |                                     |
//           +-------------------------------------+

TF_CAPI_EXPORT extern void TF_SetExecutionPriority(int execution_priority = 0);
//~wxf
